...
wru.test([
  {
    ...
    test: function () {
      ...
      setTimeout(wru.async(function(){
         var  module1 = ru(path + '/' + tmp1),
              module2 = ru(path + '/' + tmp2);
          wru.assert('same result', module1 === ru(path + '/' + tmp1) &&
                                    module2 === ru(path + '/' + tmp2));
          fs.writeFileSync('./' + tmp1, 'module.exports=Math.random()', 'utf-8');
          setTimeout(wru.async(function(){
            var newResult = ru(path + '/' + tmp1);
            wru.assert('file changed so will the result', module1 !== newResult);
            wru.assert('still cache works', newResult === ru(path + '/' + tmp1));
            wru.assert('what was cached still is', module2 === ru(path + '/' + tmp2));
            fs.unlinkSync('./' + tmp1);
            fs.unlinkSync('./' + tmp2);
          }), 1500);
      }), 1500);
    ...
    test: function () {
      ...
      var path = samePath ? '.' : '..',
          module1 = ru(path + '/' + 'tmp1.js');
      wru.assert('same results', module1 === ru(path + '/' + 'tmp1.js'));
      wru.assert('removing the cache', delete require.cache[require.resolve(path + '/' + 'tmp1.js')]);
      wru.assert('reloaded', module1 !== ru(path + '/' + 'tmp1.js'));
      wru.assert('re-cached', ru(path + '/' + 'tmp1.js') === ru(path + '/' + 'tmp1.js'));
